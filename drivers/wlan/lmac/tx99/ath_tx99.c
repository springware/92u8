...
tx99_set_cwm_param(struct ath_tx99 *tx99)
{
    if ((tx99->htmode == IEEE80211_CWM_MODE20) || (tx99->htmode == IEEE80211_CWM_MODE40)) {
        ...
        adf_os_print("%s: htmode not supported. \n", __FUNCTION__);
        return -EINVAL;
    }
    ...
    if ( tx99->htext < 3 ) { //0: extension is minus. 1: extension is plus
        if (tx99->htmode == IEEE80211_CWM_MODE40) {
            ...
            default:
                adf_os_print("tx99: extension offset in HT40 must be 1(plus), 2(minus).\n");
                return -EINVAL;
            }
...
tx99_channel_setup(struct ath_softc *sc)
{
    ...
    if (tx99->txfreq != 0) {
	    struct ieee80211_channel *c =
		    sc->sc_ieee_ops->ath_net80211_find_channel(sc, tx99->txfreq, tx99->txmode);
        if (c == NULL) {
...
{	
    ...
    tx99->txmode = IEEE80211_MODE_AUTO;
    tx99->chanmask = 1;
    tx99->recv = 0;
